# Conductor OSS 3.32.0-rc.9 Feature Matrix
...
| LIST_MCP_TOOLS | ✅ | List tools from MCP server (added 3.32.0-rc.9) |
| CALL_MCP_TOOL | ✅ | Invoke tool on MCP server (added 3.32.0-rc.9) |
| AGENT | ✅ | A2A protocol agent call; supports long-running sessions across DO_WHILE iterations (added 3.32.0-rc.9) |
| GET_AGENT_CARD | ✅ | Retrieve A2A agent capability card (added 3.32.0-rc.9) |
| CANCEL_AGENT | ✅ | Cancel in-progress AGENT task (added 3.32.0-rc.9) |
...
| AGENT | ✅ | Resume path tested across loop iterations |
...
| AGENT | ✅ | Independent executionIds per branch; official example workflows 27/33/34 demonstrate |
...
|-----------|--------|-------|-------|
| Workflow Scheduler | ✅ | pre-3.32.0 | Cron-expression scheduler. `conductor.scheduler.enabled=true` (default). Backends: SQLite, MySQL, PostgreSQL (all fully implemented). |
| HUMAN Task (basic) | ✅ | pre-3.32.0 | Task goes IN_PROGRESS; external completion via task API |
...
| Secrets Interpolation | ✅ | 3.32.0-rc.9 | Via SecretsDAO / RuntimeMetadataResolver. Default: env vars with `CONDUCTOR_SECRET_` prefix |
| Env Interpolation | ✅ | 3.32.0-rc.9 | Via EnvironmentDAO / RuntimeMetadataResolver |
| A2A Agent Protocol | ✅ | 3.32.0-rc.9 | AGENT, GET_AGENT_CARD, CANCEL_AGENT tasks; A2AWorkflowAgent server endpoint |
| LLM / AI Tasks | ✅ | pre-3.32.0 | 13 LLM providers (see table below). Enabled via AIIntegrationEnabledCondition |
| MCP Integration | ✅ | 3.32.0-rc.9 | LIST_MCP_TOOLS, CALL_MCP_TOOL tasks; MCPService |
| GraalVM JS Sandbox | ✅ | 3.30.2 | Hardened: no IO, no reflection, no native, no threads/processes, 4-second timeout |
| Python Evaluator | ✅ | pre-3.32.0 | Via GraalVM polyglot python runtime (PythonEvaluator.java) |
| Event Handlers | ✅ | pre-3.32.0 | DefaultEventProcessor + ActionProcessor |
| Workflow Variables | ✅ | pre-3.32.0 | SET_VARIABLE task + `${workflow.variables.X}` expressions |
| Input/Output Expressions | ✅ | pre-3.32.0 | `${workflow.input.X}`, `${task_ref.output.X}`, etc. |
| Workflow Metrics (Prometheus) | ✅ | pre-3.32.0 | Prometheus endpoint; includes HTTP webhook publisher metrics (PR #1149) |
| Worker Poll Visibility | ✅ | 3.32.0-rc.9 | Show worker polls in UI/API (PR #1287) |
...
## Known Fixes Landed in 3.31.x / 3.32.0-rc.9

| Fix | Commit / PR |
|-----|------------|
| DO_WHILE + SWITCH premature iteration advancement | commit 69faed274, PR #1001 |
| SWITCH empty-case no longer falls through to defaultCase | PR #1159 (3.31.0) |
| DO_WHILE iteration list truncation | PR #1172 (3.31.0) |
| Nested JOIN transitive reset after sub-workflow restart | PR #1212 (3.31.0) |
...
| Conductor Agents (AGENT task) support | PR #1288 |
| Prometheus metrics for HTTP webhook publishers | PR #1149 |
| Worker poll visibility | PR #1287 |
